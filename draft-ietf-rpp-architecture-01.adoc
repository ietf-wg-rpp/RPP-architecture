...
In order to enable effective processing of the messages, the data model of a message defines metadata fields with a type of a message and extension or a profile used to produce it, so that the clients know how to process message payload.

At minimum, RPP defines message queue resources that clients can access to discover and retrieve pending service messages. The baseline interaction pattern follows a read-first/acknowledge-delete approach:
...
RPP offers separate queues per message type, so that clients may split the workload to several specialised readers, however a joint queue of all messages is offered for convenience and EPP compatibility.
...
Additionally, RPP may support or be further extended with optional features for enhanced service message handling:

*  retrieve/acknowledge/delete multiple service messages in a single request, reducing round trips for clients with many pending messages.
*  Supporting multiple concurrent readers, allowing different client sessions or applications to access the message queue simultaneously.
*  Supporting streaming mechanisms (such as Server-Sent Events or WebSockets) for real-time delivery of service messages to clients, as an alternative to polling-based retrieval.
