...
At minimum, RPP defines a single message queue resource that clients can access to discover and retrieve pending service messages. The baseline interaction pattern follows a read-first/acknowledge-delete approach:
...
3.  The client explicitly acknowledges the message through a DELETE operation, removing it from the queue.

In order to assure idempotency and proper processing in case of concurrent clients, each service message must carry a unique message id, which allows clients to assure the correct message is acknowledged and deleted in step 3.

Information about unread messages may be delivered as a custom http header with each queue response. Such header can be also added to other responses (for example with the first response after the queue state change or every n-th response or time based) in order to give a hint to the client about unread messages, however considering possible negative impact on server operation and performance.

The baseline mechanism may be extended through:

*  Separate queues per message type, where servers may expose distinct queue subresources for different categories of service messages, allowing clients to query specific message types independently.
*  Message filtering, where clients may apply filters using query parameters when querying the message queue to retrieve only messages matching specific criteria.

Additionally, RPP may support or be further extended with optional features for enhanced service message handling:

*  Allowing clients to retrieve multiple service messages in a single request, reducing round trips for clients with many pending messages.
