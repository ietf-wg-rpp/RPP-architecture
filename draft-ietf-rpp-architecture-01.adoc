...
RPP may optionally include support for a simplified object transfer process, using an extension. The protocol must allow for a future extension where approval from the losing registrar can be obtained interactively by the registrant during the transfer process, rather than relying solely on asynchronous or out-of-band mechanisms.
This approach enables a more streamlined and user-friendly transfer experience and reducing delays. RPP should allow for interactive workflows, such as real-time notifications or approval prompts, to facilitate immediate action by the registrant and registrar.
Implementations supporting this feature should ensure secure authentication and authorization of all parties, and provide clear audit trails of the approval process.
...
* Added text about simplified object transfers to "Object level authorisation" section. (Issue #32)
