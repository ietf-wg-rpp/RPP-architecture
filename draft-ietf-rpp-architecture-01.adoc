...
For HTTP Basic Authentication revocation must be fulfilled by off-band operational practice, or using extension as described in <<credential-management>> to rotate the credential.
Protocol specifications must emphasise, that implementations should ensure that revocation takes effect promptly and that revoked credentials are consistently rejected across all RPP endpoints.
...
All parties involved in a running or completed process, such as the Sponsoring Client and the Gaining Client for a transfer operation, must be allowed to query the status of the process using the same process resource. 
...
* Internationalised identifiers (IDNs) and their ASCII/Punycode equivalents
...
When a resource is addressed using an alias, RPP must ensure canonicalisation through at least one of the following mechanisms:

* Redirecting the client to the canonical URL
* Returning a representation that explicitly declares the canonical URL
* Internally resolving the alias to the canonical identifier before any authorisation, or state mutation occurs

Canonicalisation shall be performed prior to any operation that depends on resource identity.
...
RPP shall use as specific HTTP response status codes as possible, using generic codes like 400 Bad Request or 500 Internal Server Error only if no corresponding specific code exists (e.g. 404 for not existing resource or 401 for unauthenticated request).
...
RPP uses a dual-layer approach for signalling operation outcomes, leveraging both standard HTTP response status codes and RPP-specific status codes. This allows for compatibility with generic HTTP components while providing detailed, application-level feedback for RPP clients.
...
This enables robust modelling, navigation, and management of complex, interconnected resources within RPP.
...
** Refactoring of <<extension-mechanisms, format=title>> Section and addition of extensibility with transient request parameters and responses (Issues #94 and #95)
...
* Collections, Bulk Operations & Filtering
...
* Resource Definition & Data Model
